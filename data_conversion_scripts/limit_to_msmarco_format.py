...
if __name__ == "__main__":
    ...
    with open("../limit/limit/queries.jsonl", "r") as infile, open("../limit_formatted/limit/queries.tsv", "x") as outfile:
        for line in infile:
            data = json.loads(line)
            outfile.write(f"{data['_id']}\t{data['text']}\n")

    # Convert limit corpus to msmarco tsv format
    with open("../limit/limit/corpus.jsonl", "r") as infile, open("../limit_formatted/limit/corpus.tsv", "x") as outfile:
        for line in infile:
            data = json.loads(line)
            outfile.write(f"{data['_id']}\t{data['text']}\n")

    # Convert limit qrels to msmarco tsv format
    with open("../limit/limit/qrels.jsonl", "r") as infile, open("../limit_formatted/limit/qrels.json", "x") as outfile:
        qrels = {}
        for line in infile:
            data = json.loads(line)
